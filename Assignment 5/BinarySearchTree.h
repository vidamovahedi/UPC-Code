...
public:
	...
	void insert(T& item, string fill) { insert(root, item, fill); }
	string search(T& item) { return search(root, item); }
...
private:
	...
	int height(node<T>*);
	int max(int, int);
	void insert(node<T>*&, T&, string fill);
	string search(node<T>*, T&);
	T findMax(node<T>*, T&);
	T findMin(node<T>*, T&);
...
template <class T>
void BinarySearchTree<T>::insert(node<T>*& p, T& item, string filler) {
	if (p == NULL) {
		p = new node<T>;
		p->data = item;
		p->fill = filler;
		...
		insert(p->left, item, filler);
	}
	else {
		insert(p->right, item, filler);
	}
}

template <class T>
string BinarySearchTree<T>::search(node<T>* p, T& item) {
	if (p == NULL)
		return "";
	else if (item<p->data)

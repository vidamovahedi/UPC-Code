...
public:
	...
	void insert(T& item,string fill){insert(root, item,fill);}//edited to accept extra data
...
private:
	...
	int height(node<T>*);
	int max(int, int);
	void insert(node<T>*&, T&, string&);//edited to accept extra data
...
template <class T>
void BinarySearchTree<T>::insert(node<T>*& p, T& item, string& fill){
	if(p == NULL){
		p = new node<T>;
		p->data = item;
		p->filler = fill;
